...
class TestPlayingDeck {
    ...
    @Test
    fun `when number of cards asked for is 0, returned hand should contain 0 card`() {
        val uut = PlayingDeck()
        uut.add(StandardDeck())
        val expected = 0

        val hand = uut.takeHand(expected)
        val actual = hand.cards.count()

        Assert.assertEquals(expected, actual)
    }

    @Test
    fun `when number of cards asked for is 1, returned hand should contain 1 card`() {
        val uut = PlayingDeck()
        uut.add(StandardDeck())
        val expected = 1
    ...
    @Test
    fun `when number of cards asked for is 4, returned hand should contain 4 card`() {
        val uut = PlayingDeck()
        uut.add(StandardDeck())
        val expected = 4
